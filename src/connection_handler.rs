...
impl NetState {
    pub(crate) fn on_connection_packet(&self, mut data: &[u8]) -> Result<(), Box<dyn Error>> {
    ...
    fn on_login_request(&self, mut data: &[u8]) -> Result<(), Box<dyn Error>> {
    ...
    pub fn send_login_response(&self, state: LoginState) -> Result<(), Box<dyn Error>>{
        ...
        writer.put_u16_le(self.map_ref_mut().width());
        writer.put_u16_le(self.map_ref_mut().height());
    ...
    fn on_quit(&self, mut _data: &[u8]) -> Result<(), Box<dyn Error>>{
