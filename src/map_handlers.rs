...
impl NetState{
    pub(crate) fn on_blocks_request_packet(&self, cedserver: &mut CedServer, mut data: &[u8]) -> Result<(), Box<dyn Error>> {
        ...
        for _i in 0..blocks_count {
            self.send_block_packet(&mut cedserver.map, data.get_point_u16())?;
        }
        //TODO: Zlib compress
    ...
    pub fn send_block_packet(&self, map: &mut Map, point: PointU16) -> Result<(), Box<dyn Error>> {
